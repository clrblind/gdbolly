...
*   **Архитектура (v0.1.0 Refactor)**:
    *   **Modular Routers**: API разделен на логические модули в `backend/app/routers/` (`session`, `control`, `memory`, `settings`).
    *   **GDB Package**: Логика управления отладчиком вынесена в отдельный пакет `backend/gdb/` (Singleton Controller).
    *   **Asyncio & Queues**: Полностью асинхронное взаимодействие через очереди сообщений.
    *   **Token-Based Request Matching**: Механизм синхронизации команд через UUID токены для гарантии атомарности операций (чтение/запись памяти).
