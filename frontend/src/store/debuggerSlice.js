...
const initialState = {
    ...
    registers: [],
    registerNames: [], // Array of names, index = number
    disassembly: [],
...
export const debuggerSlice = createSlice({
    ...
    reducers: {
        ...
        updateRegisters: (state, action) => {
            state.registers = action.payload.map(reg => ({
                ...reg,
                name: state.registerNames[parseInt(reg.number)] || null
            }));
        },
        setRegisterNames: (state, action) => {
            state.registerNames = action.payload;
            // Backfill names if registers already exist (race condition fix)
            if (state.registers.length > 0) {
                state.registers = state.registers.map(reg => ({
                    ...reg,
                    name: state.registerNames[parseInt(reg.number)] || null
                }));
            }
