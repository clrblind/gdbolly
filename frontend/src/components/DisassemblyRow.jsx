...
const Mnemonic = styled.span`
  color: ${props => props.$selected ? '#fff' : props.$modified ? '#ff0000' : props.$isJmp ? '#2d5016' : props.$isCall ? '#0000ff' : props.$isRet ? '#ff0000' : '#000'};
  font-weight: ${props => (props.$isCall || props.$isRet || props.$isJmp) ? 'bold' : 'normal'};
`;
...
const DisassemblyRow = ({
  ...
  const isJmp = mnemonic.toLowerCase() === 'jmp';
  ...
  return (
    ...
    >
      ...
      <Cell style={{ width: colWidths[2] }}>
        <Mnemonic $isCall={isCall} $isRet={isRet} $isJmp={isJmp} $selected={isSelected} $modified={isModified}>{mnemonic}</Mnemonic>
        &nbsp;
